            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
              ...
              <li>
                <div class="flex items-center">
                  ...
                  <a href="/sellers" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">Seller</a>
                </div>
              </li>
              <li aria-current="page">
                <div class="flex items-center">
                  <svg aria-hidden="true" class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                  <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ __($seller->name) }}</span>
                </div>
              </li>
            </ol>
          </nav>
        <h2 class="font-semibold text-2xl text-gray-800 leading-tight ">
           Seller - {{ __($seller->name) }}
        </h2>
    </x-slot>
    ...
    <div class="pb-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 relative">   
            ...
            <form class="md:w-[50%] submit-form"
                data-method="POST"
                data-action="/seller/{{ $seller->id }}"
                data-callback="show"
            >
                <x-field id="name" name="name" type="text" label="Name" value="{{ __($seller->name) }}"></x-field>   
                <x-button type="submit">Save</x-button>
            </form>

            <form id="dropdown" data-method="DELETE" data-action="/seller/{{ $seller->id }}"
                data-callback="redirect" class="submit-form inline">
